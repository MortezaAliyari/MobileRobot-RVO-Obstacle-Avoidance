                                           ...
                                           double* min_y, double* max_x, double* max_y)
{
    ...
    /*
The updateBounds method does not change the costmap just yet. It just defines the area that will need to be updated.
We calculate the point we want to change (mark_x_, mark_y_) and then expand the min/max bounds to be sure it includes the new point
*/
  if (!enabled_)
    return;
...
void SimpleLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
                                          int max_j)
{
    //here  the four sides of truncated cone drawn . each for loop related to one side of shape.
  if (!enabled_)
    return;
  unsigned int mx;
  unsigned int my;
...
if(flag==true && abs(dis)>0.6){ // drwan shape with two reasons: first reason correspond to recieving data from agents and second is minimum distant from obstacle (shape ruined if agent go closer).
if (sin(losangle)>=0){
